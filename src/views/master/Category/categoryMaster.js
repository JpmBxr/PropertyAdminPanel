...
export const categoryMaster = {
  ...
  methods: {
    ...
    get() {
      ...
      sortBy = sortBy.length == 0 ? "product_category_name" : sortBy[0];
      let payload = {
        sortColumn: sortBy,
        sortOrder: sortDesc,
      ...
      ApiService.get("GetProductCategory", payload)
        .then((response) => {
    ...
    showAddEditDialog(item) {
      if (item == null && this.isAddEdit == true) {
        ...
        this.addEditText =
          `Edit ${this.entity} : ` + this.item.product_category_name;
        this.addEditDialog = true;
    ...
    addEditItem() {
      if (this.$refs.holdingFormAddEdit.validate()) {
        if (this.isAddEdit) {
          // save
          let payload = {
            product_category_name: this.item.product_category_name,
          ...
          let payload = {
            ...
            product_category_status: this.item.product_category_status,
          };
          this.isDialogLoaderActive = true;
    ...
    async enableDisableItem(item) {
      const result = await Global.showConfirmationAlert(
        `Change ${this.entity} : ${item.product_category_name} Status`,
      ...
      if (result.isConfirmed) {
        let payload = {
          ...
          product_category_status:
            this.item.product_category_status == "Active"
              ? "Active"
              : "InActive",
        };
        this.isLoaderActive = true;
    ...
    async deleteItem(item) {
      const result = await Global.showConfirmationAlert(
        `Delete Role ${item.product_category_name}`,
