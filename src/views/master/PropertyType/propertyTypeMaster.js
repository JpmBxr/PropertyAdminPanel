...
export const propertyTypeMaster = {
  ...
  methods: {
    ...
    get() {
      ...
      sortBy = sortBy.length == 0 ? "property_type" : sortBy[0];
      let payload = {
        sortColumn: sortBy,
        sortOrder: sortDesc,
      ...
      ApiService.get("GetPropertyType", payload)
        .then((response) => {
    ...
    showAddEditDialog(item) {
      if (item == null && this.isAddEdit == true) {
        ...
        this.addEditText = `Edit ${this.entity} : ` + this.item.property_type;
        this.addEditDialog = true;
    ...
    async enableDisableItem(item) {
      const result = await Global.showConfirmationAlert(
        `Change ${this.entity} : ${item.property_type} Status`,
      ...
      if (result.isConfirmed) {
        let payload = {
          ...
          property_type_status:
            item.property_type_status == "Active" ? "Active" : "InActive",
          updated_by: Global.loggedInUser,
        };
        this.isLoaderActive = true;
        ApiService.post(`Update${this.endPoint}`, payload)
        ...
        if (item.property_type_status == "Inactive") {
          item.property_type_status = "Active";
        } else {
          item.property_type_status = "Inactive";
        }
      }
    },
    ...
    async deleteItem(item) {
      const result = await Global.showConfirmationAlert(
        `Delete  ${this.entity} : ${item.property_type}`,
